...
export async function searchJamendoTracks(query: string, limit: number = 50): Promise<Track[]> {
    try {
        ...
        logger.info('Searching Jamendo:', query);
        const response = await fetch(url); if (!response.ok) {
            logger.error(`❌ Jamendo search failed: ${response.status}`);
            return [];
        }
        ...
        logger.info(`Found ${tracks.length} Jamendo tracks (FULL LENGTH)`);
        return tracks;
    } catch (error) {
        logger.error('❌ Jamendo search error:', error);
        return [];
    }
}
...
export async function getJamendoPopularTracks(limit: number = 50): Promise<Track[]> {
    try {
        ...
        logger.info('Loading Jamendo popular tracks...');
        const response = await fetch(url);

        if (!response.ok) {
            logger.error(`❌ Jamendo popular tracks failed: ${response.status}`);
            return [];
        }
        ...
        logger.info(`Loaded ${tracks.length} Jamendo popular tracks (FULL LENGTH)`);
        return tracks;
    } catch (error) {
        logger.error('❌ Jamendo popular tracks error:', error);
        return [];
    }
}
...
export async function getJamendoTracksByGenre(genre: string, limit: number = 50): Promise<Track[]> {
    try {
        ...
        logger.info(`Loading Jamendo ${genre} tracks...`);
        const response = await fetch(url);

        if (!response.ok) {
            logger.error(`❌ Jamendo genre tracks failed: ${response.status}`);
            return [];
        }
        ...
        logger.info(`Loaded ${tracks.length} ${genre} tracks (FULL LENGTH)`);
        return tracks;
    } catch (error) {
        logger.error(`❌ Jamendo ${genre} tracks error:`, error);
        return [];
    }
}
